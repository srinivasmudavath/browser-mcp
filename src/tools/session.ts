...
const sessionInfo = defineTool({
  ...
  handle: async (context, params, response) => {
    const sessionManager = SessionManager.getInstance(context['_browserContextFactory']);
    const sessionInfo = sessionManager.getSessionInfo();
    const currentSessionId = context['_sessionId'] || 'default';
...
const sessionCreate = defineTool({
  ...
  handle: async (context, params, response) => {
    ...
    try {
      ...
      const browserContext = await sessionManager.getOrCreateSession(params.sessionId);
...
const sessionClose = defineTool({
  ...
  handle: async (context, params, response) => {
    ...
    try {
      const success = await sessionManager.closeSession(params.sessionId);
      
      if (success) {
        response.addResult(`✅ Browser session closed successfully!`);
        response.addResult(`Session ID: ${params.sessionId}`);
      } else {
        response.addResult(`⚠️ Session not found or already closed: ${params.sessionId}`);
      }
...
const sessionCleanup = defineTool({
  ...
  handle: async (context, params, response) => {
    ...
    try {
      const cleanedCount = await sessionManager.cleanupOldSessions(params.maxAgeMinutes || 60);
